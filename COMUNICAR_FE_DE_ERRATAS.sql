...
create table #t (coso int)
INSERT INTO #T VALUES(1)

SELECT * FROM #T

UPDATE Cartera set name = case when name = 'une376' then 'une342' when name = 'une342' then 'une376' else name END FROM cartera

insert into cartera
select d_party, asistente, triada, usuario from tempdb..nuevas_carteras where usuario in ('une376','une342')


delete from cartera where cd_triada in (select triada from tempdb..nuevas_carteras)
rollback tran
select * from cartera where name in ('une376','une342')